...
func init() {
	...
	_ = envoyv1alpha1.AddToScheme(scheme)
...
func main() {
	...
	if err = (&controllers.RouteConfigurationReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("RouteConfiguration"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "RouteConfiguration")
		os.Exit(1)
	}
	if err = (&controllers.ScopedRouteConfigurationReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("ScopedRouteConfiguration"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ScopedRouteConfiguration")
		os.Exit(1)
	}
